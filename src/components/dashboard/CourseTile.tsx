...
export function CourseTile({ course, index }: { course: Course; index: number }) {
  ...
  return (
    <BentoTile glow className="min-h-[200px] p-6 grain flex flex-col justify-between">
      ...
      />
      {/* Animated gradient border on hover — opacity-only */}
      <span
        aria-hidden
      ...
      <header className="relative flex items-start justify-between gap-4">
        <div
          ...
          style={{ boxShadow: "0 0 20px -8px var(--color-border-glow)" }}
        >
          <Icon className="size-5 text-primary-glow" aria-hidden />
        </div>
      ...
      <div className="relative mt-6">
        <h3 className="text-lg font-semibold text-foreground leading-snug">{course.title}</h3>
        ...
        >
          <motion.div
            ...
            transition={{ type: "spring", stiffness: 60, damping: 18, delay: 0.3 + index * 0.08 }}
